...
class Paramaters {
	...
	readParameter(greedy = false, offset = true) {
		var value = null;
		if (!this.hasFinished()) {
	...
	readNumber(offset = true) {
		var param = this.readParameter(false, offset);
		var value = null;
		if (param != null) {
	...
	readObject(objects, name = "name", mentions = /(.+)/, allowPartial = true, filter = () => { return true; }, offset = true) {
		var param = (this.readParameter(true, offset) || "").toLowerCase();
	...
	readUser(allowPartial = true, filter = () => { return true; }, offset = true) {
		return this.readObject(this.client.users, "username", /<@!?(\d+)>/, allowPartial, filter, offset);
	}

	readRole(allowPartial = true, filter = () => { return true; }, offset = true) {
		return this.readObject(this.guild.roles, "name", /<@&(\d+)>/, allowPartial, filter, offset);
	}

	readChannel(allowPartial = true, filter = () => { return true; }, offset = true) {
		return this.readObject(this.guild.channels, "name", /<#(\d+)>/, allowPartial, filter, offset);
	}

	readMember(allowPartial = true, filter = () => { return true; }, offset = true) {
