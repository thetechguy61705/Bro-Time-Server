					...
					message.react("👍").then((upVote) => {
						...
						var getTally = () => {
							...
							message.reactions.forEach((reaction) => {
								tally += reaction === upVote ? reaction.users.size : -reaction.users.size;
							});
							return tally;
						};
						var updateMessage = (reaction) => {
							...
							if (reaction.message.id === message.id)
								message.edit(replaceTags(content, [{tag: "<current>", value: Math.max(tally, 0)}, {tag: "<required>", value: required}]));
							if (tally >= required)
								finish();
						};
						finish = () => {
