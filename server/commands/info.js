...
async function gameRoles(message, Discord, prompt, param) {
	...
	if(prompt.toLowerCase() === "preview") {
		...
		message.channel.send(gameRoleEmbed).then(async function(embedMessage) {
			...
			message.author.send(`You attempted to run the command \`info\` in ${message.channel}, but I could not chat there.`).catch();
		});
	} else if(prompt.toLowerCase() === "specify") {
		awaitReply(message, "What game role do you want info on?").then((userChoice) => {
			prompt2 = games.find(function(role) {
				return role.toLowerCase().substr(1).slice(0, -1).startsWith(userChoice.toLowerCase());
			});
			if(prompt2 !== undefined) {
				currentRole = message.guild.roles.find("name", userChoice.substr(1).slice(0, -1));
				let roleEmbed = new Discord.RichEmbed()
					.setTitle(userChoice.substr(1).slice(0, -1))
					.setDescription(`Players: \`${currentRole.members.size}\``)
					.setColor(currentRole.hexColor);
...
async function nameColors(message, Discord, prompt, param) {
	...
	if(prompt.toLowerCase() === "preview") {
		...
		message.channel.send(nameColorEmbed).then(async function(embedMessage) {
		 ...
		 awaitReply(message, "What name color role do you want info on?").then((userChoice) => {
...
async function howToGetRole(message, Discord, prompt, param) {
	...
	if(prompt.toLowerCase() === "preview") {
		...
		awaitReply(message, "What obtainable role do you want info on?").then((userChoice) => {
...
async function levelRoles(message, Discord, prompt, param) {
	...
	if(prompt.toLowerCase() === "preview") {
		...
		awaitReply(message, "What level role do you want info on?").then((userChoice) => {
...
async function infoTarget(message, prompt, Discord, choice, param) {
	if(choice.toLowerCase() === "ad" || choice.toLowerCase() === "advertisement") {
		if(param !== "mobile" && param !== "computer") {
			awaitReply(message, "Would you like to view the `mobile` ad (not in code block) or `computer` ad (in code block)? Default: Computer").then((userChoice) => {
				ad(message, userChoice, undefined);
			});
		} else {
		...
		if(param !== "preview" && param !== "list" && param !== "specify") {
			awaitReply(message, "Would you like to `preview` all game roles, view a `list` of game roles or `specify` a specific role? Default: List").then((userChoice) => {
				gameRoles(message, Discord, userChoice, undefined);
			});
		} else {
		...
		if(param !== "preview" && param !== "list" && param !== "specify") {
			awaitReply(message, "Would you like to `preview` all color roles, view a `list` of color roles or `specify` a specific role? Default: List").then((userChoice) => {
				nameColors(message, Discord, userChoice, undefined);
			});
		} else {
		...
		if(param !== "preview" && param !== "list" && param !== "specify") {
			awaitReply(message, "Would you like to `preview` all obtainable roles, view a `list` of roles or `specify` a specific role? Default: List").then((userChoice) => {
				howToGetRole(message, Discord, userChoice, undefined);
			});
		} else {
		...
		if(param !== "preview" && param !== "list" && param !== "specify") {
			awaitReply(message, "Would you like to `preview` all obtainable roles, view a `list` of roles or `specify` a specific role? Default: List").then((userChoice) => {
				levelRoles(message, Discord, userChoice, undefined);
			});
		} else {
