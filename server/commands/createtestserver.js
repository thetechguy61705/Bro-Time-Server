...
module.exports = {
	...
	execute: (call) => {
		...
		realGuild.channels.forEach(function(channel) {
			testGuild.createChannel(reverse(channel.name), channel.type, channel.positionOverwrites).then(newChannel => {
		...
		realGuild.roles.forEach(function(role) {
			testGuild.createRole(role.name, role.color, role.hoist, role.permissions, role.mentionable).then(newRole => {
