	...
	run: (call, actions, parameter) => {
		...
		if (!isNaN(amountToPurge) && amountToPurge > 0 && amountToPurge <= 100) {
			call.message.delete().then(() => {
				call.purgeMessages(amountToPurge, call.message.channel, (msg) => msg.author.id === (user || { id: null }).id).then((amountPurged) => {
			...
			call.message.reply("Please specify a number above 0 and below 500 or a purge action from one of the following: `" +  actions.keyArray().join("`, `") + "`.").catch(() => {
